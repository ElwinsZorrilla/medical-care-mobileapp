...
| RF-11 | Estado de verificación | F05,F17 | `BadgeVerificacion` — color + glifo + etiqueta + **explicación** de qué implica. El lado admin —`PATCH /doctors/:id/verificacion`, `features/admin/presentation/screens/verificacion_screen.dart`— es lo que mueve el estado; antes no había forma de sacar un médico de PENDIENTE | 3 estados con prueba de widget + pruebas de repositorio/provider/pantalla del lado admin | ✅ |
...
| RF-24 | Mis citas / mi agenda | F08,F17 | Una pantalla, dos rutas según rol; scroll infinito y caché de médicos, y —desde F17— caché simétrica de pacientes (`PacienteDirectorio`) para que la agenda pinte el nombre en vez de `Paciente #7` (BACKEND_ISSUES.md #10) | pruebas de los 4 estados y de ambos roles | ✅ |
...
| RNF-14 | Pruebas por modulo | todas,F13,F15,F16,F17 | **84.8 %** de linea excluyendo codigo generado —81.9 % contando todo—, 664 pruebas (660 mas 4 goldens que solo corren en Linux). Las dos cifras las imprime `verify.sh`, que ademas falla si este documento o el README afirman otra. Se agrego porque los dos se quedaron tres fases con una cifra vieja sin que nada lo dijera. En F16 se cerraron las dos deudas declaradas en la revision de F10: `bandeja_screen.dart` paso de 1/87 lineas a cubierta, y las capas `api`/`dto` —que los dobles de prueba dejaban sin ejecutar— entraron a `contrato_api_test.dart`, donde la ruta literal y el `fromJson` se ejercen de verdad. En F17 se sumaron `guardar_paciente_test.dart` (reproduce el `Ref` desechado antes de arreglarlo) y las pruebas de los features `admin` y de la resolucion de nombre de paciente en la agenda. Ver `docs/HARDENING.md` | ✅ |
| RNF-15 | Swagger | F00,F16,F17 | `docs/openapi.json` — **40 endpoints**, extraído de `/docs-json` del servidor real. Eran 29 hasta que el backend agregó notificaciones, chat y video; 40 desde que se agregaron `PATCH /doctors/{id}/verificacion` y `GET /patients/{id}` | ✅ |
| RNF-16 | Errores claros y uniformes | F03 | Jerarquía sellada `Failure` + `FailureMapper`; 21 pruebas de mapeo HTTP→dominio | ✅ |
| RNF-17 | Docker | F14 | `docker build --target verify` termina en 0: **664 pruebas, cobertura 84.8 %** dentro del contenedor. Targets `verify`, `goldens` y `web`; `artifacts`/`export` eliminados porque nunca compilaron (sin SDK de Android). El APK se compila en el job `apk` del CI | ✅ |
